          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuAttr, index) in spuSaleAttrList" :key="spuAttr.id">
                <dt class="title">选择{{ spuAttr.saleAttrName }}</dt>
                <dd changepirce="0" v-for="(value, index) in spuAttr.spuSaleAttrValueList" :key="value.id"
                  :class="{ active: value.isChecked == 1 }" @click="changeActive(value, spuAttr.spuSaleAttrValueList)">
  ...
  methods: {
    ...
    async addShopcart() {
      try {
        ...
        // 路由跳转
        // 在路由跳转时还需要将产品的信息带给下一级 的 addCartSuccess路由组件
        // 一些简单的数据skuNum ,可以通过query参数形式给路由组件传递过去
        // 产品信息 数据复杂 通过会话存储 传递 ,本地|会话存储的是字符串,首先要将对象转字符串JSON.
