            ...
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="(goods, index) in goodsList" :key="goods.id">
                <div class="list-wrap">
                  ...
                  <div class="attr">
                    <a target="_blank" href="item.html" title="">{{ goods.title }}</a>
                  </div>
                  <div class="commit">
...
export default {
  ...
  computed: {
    // mapState 只能用对象写法 因为 state 划分了模块home search
    ...mapState({ searchList: state => state.search.searchList, total: state => state.search.searchList.total }),
    // mapGetters 数组写法 因为 getters 没有划分模块
  ...
  methods: {
    ...
    // 删除关键字
    removeKeyword() {
      // 给服务器带的参数searchParams中的keyword置空
    ...
    // 子组件给父组件传递信息 获取点击的品牌信息的自定义事件 的回调
    trademarkInfo(trademark) {
      // console.log('search父组件', trademark);
      // 整理品牌字段的参数
    ...
    changeOrder(flag) {
      // flag形参 标记用户点击的是 综合（1）还是价格（2） 用户点击的时候传递进来
      let originOrder = this.searchParams.order
      let originFlag = originOrder.split(":")[0]
      let originSort = originOrder.split(":")[1]
      // console.log(flag); // 数字
      // console.log(originFlag); // 字符串，因此下面判断两个flag只能用 双等
